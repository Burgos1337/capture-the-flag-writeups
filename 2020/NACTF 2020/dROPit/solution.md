...
`No canary found` - стековая канарейка отсутствует, что значительно упрощает наш алгоритм переполнения буфера;
...
Перед написанием эксплойта определим алгоритм эксплуатации обнаруженной уязвимости:
- переполняем буфер до адреса возврата;
- при помощи `ROP`-гаджета перемещаем адрес функции `puts@got` в регистр `RDI` 
